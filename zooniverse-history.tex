...
\subsubsection{Intrinsic Interestingness and Task Difficulty} 
...
Overall, the team concluded that, among these three factors, the two of \emph{interesting-ness of subject} and 
\emph{difficulty of task} dominated the others such as type of task, domain of origin, source of data.  There was no perceived advantage of one research field over another and projects within the same field often did better than others; for example, astronomy projects such as XX maintained attention and engagement while participation in YY, also in astronomy leveled off after an initial period after launch.   Furthermore, one task type was not particularly better received than others; for example, classification or marking tasks were often successful, but so were transcription tasks (e.g., Old Weather).

We discuss interesting-ness and task difficulty here in some depth.
...
\item \emph{Interestingness/Richness} - 

\item \emph{Task Difficulty} - 
...
Towards this goal, the team identified what at least two key factors - the intrinsic ``interesting-ness'' of the subject to participants, and the overall difficulty of the task.  Pertaining to the first, there were many examples of projects which had an abundance of beautiful photos that appealed to the general audience, and it was these projects that exhibited the greatest user rention as well as attracted the most users.  In particular, Galaxy Zoo, which features a large number of beautiful photos of galaxies and deep space, and Snapshot Serengeti, which features an abundance of ``candid'' photos of popular animals in the wild were each 

Tasks, however that were not particularly interesting but can still sustain engeagement if they are \emph{easy}.
...
The final theme we discuss here pertain to launching projects and outreach efforts, including the use of social media, and the ways that the degree of involvement of the science team impacts 
