...
However, designing successful citizen science systems that can be mutually beneficial in this way while sustaining participation over time can be exceedingly difficult.  The reasons are several: first, due to the emerging nature of the field, key design principles and \emph{best practices} are not yet known or established.  Moreover, despite considerable overlap with human computation systems, key differences in the ways that people interact with citizen science applications have implications for the design of each. For example, human computation systems often apply extrinsic motivation (e.g., direct rewards) to drive participation, while most citizen scientists are compelled to particpiate for their own intrinsic reasons\cite{extrinsic-vs-intrinsic}. Since such motivations are typically personal and idiosyncratic, understanding ways to address and engage these motivations can be prohibitive\cite{raddick2010galaxy}.  Participants typically engage with citizen science systems in many different ways beyond merely performing tasks, such as by taking part in discussions, interacting with the Science teams, sharing subjects they found interesting, and so forth, meaning that measuring 'success' itself can be diffficult. Finally, the simple fact that, over time, the more time has elapsed simply forget about any particular system, means that retaining experienced particpants over time.
...
In this paper, we contribute a detailed case-study of a unique citizen-science platform which expanded from a single domain experiment to a ``factory for citizen science'', an authority for generating successful web applications for scientists across domains.  The iterative process of creating apps for particular science projects, culimating in 27 projects total by September 2013, allowed the team to explore design variations at mutiple levels, from the interface/intraction flow, to the design of social elements (discussion forums), to external considerations such as ways to best launch and sustain prolonged engagement through external communications. Zooniverse also represents the first system in which volunteer contributions directly resulted in the publication of scientific findings in academic journals, contributing findings to at least $P$ published scientific papers.  

The analysis presented here is the result of a retrospective reflective thematic analysis towards identifying key insights and consequences of design decisions gained through the trial-and-error process of generating apps for various domains and data-sets since Galaxy Zoo, the first Zooniverse application.  The goal of the this analysis was to document the informal knowledge gathered by the Zoonvierse team pertaining to how to design successful citizen science projects.  These insights were then used to derive design recommendations and discuss factors that may be responsible for the observations made. 
