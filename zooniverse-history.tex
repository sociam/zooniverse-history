...
Overall, the team concluded that, among these three factors, the two of \emph{interesting-ness of subject} and \emph{difficulty of task} dominated the others such as type of task, domain of origin, source of data.  There was no perceived preference to one research field's tasks over another, and substantial variability among projects within the same field meant that othe factors were the primary forces at play; for example, astronomy projects such as XX maintained attention and engagement while participation in YY, also in astronomy levelled off after an initial period after launch.   Furthermore, one task type was not particularly better received than others; for example, classification or marking tasks were often successful, but so were transcription tasks (e.g., Old Weather).

\subsection{Difficulty and Role of reward}
