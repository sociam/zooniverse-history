...
\subsubsection{Intrinsic Interestingness vs Task Difficulty} 
...
Overall, the team concluded that, among these three factors, the two of \emph{interesting-ness of subject} and \emph{difficulty of task} dominated the others such as type of task, domain of origin, source of data.  There was no perceived advantage of one research field over another and projects within the same field often did better than others; for example, astronomy projects such as XX maintained attention and engagement while participation in YY, also in astronomy leveled off after an initial period after launch.   Furthermore, one task type was not particularly better received than others; for example, classification or marking tasks were often successful, but so were transcription tasks (e.g., Old Weather).

We discuss interesting-ness and task difficulty here in some depth.  

\begin{itemize}

\item \emph{Interestingness} - The intrinsic ``interesting-ness'' of the subjects being classified to participants played a major role in attracting and inspiring individuals to continue to participate.  many examples of projects which had an abundance of beautiful photos that appealed to the general audience, and it was these projects that exhibited the greatest user retention as well as attracted the most users.  In particular, Galaxy Zoo, which features a large number of beautiful photos of galaxies and deep space, and Snapshot Serengeti, which features an abundance of ``candid'' photos of popular animals in the wild were each 
...
The additional information provided for the Old Weather project has not only helped attract users to the project, but has offered valuable data in its own right. 
