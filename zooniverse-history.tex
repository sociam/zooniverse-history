...
Table \ref{table:project-summary} lists all Zooniverse projects by their launch date, their current status, web address, number of subjects being processed, and task type\footnote{all statistics are as of September 2013; the active projects may be extended to new data sets so the number of subjects is likely to increase through time}. \emph{Retired} projects have achieved their classification goals and have been closed for new contributions. \emph{Active} projects are those still open for new contributions.  Projects generally have one or more kinds of tasks; \emph{classifying} tasks ask users to identify the presence, types, and potentially number of items visible in each subject.  An example is the task of identifying and counting animals of \emph{Snapshot Serengeti}. \emph{Marking} tasks involve the additional step of indicating where the particular object was found in an image subject, for example, indicating the locations of organisms in undersea images of \emph{Seafloor Explorer}.  Finally, \emph{transcribing} tasks require reading ancient, such as the ship logs in \emph{Old Weather} or museum specimen registers in \emph{Notes from Nature}. Figure 1, likewise, illustrates the growth of the project from June 2007 until September 2013.  
